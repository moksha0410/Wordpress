...
if (!class_exists('\HelpieFaq\Features\Insights\Trackers\Search_Tracker')) {
...

    {
        ...
        public function get_new_count($counter_data, $event_data)
        {
            ...
            if ($this->is_search_empty($event_data)) {
                return $counter_data;
            }
        ...
        {
            $search_term = trim($string);
            $search_term = strtolower($search_term);

            $searched_words = array();
            // add count for the whole string
            $searched_words[$search_term] = 1;

            $search_term = $this->removeCommonWords($search_term);
            $words = preg_split('/\s+/', $search_term);
            $words = array_map('trim', $words);
            // add count for each word
